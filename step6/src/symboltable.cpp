...
void Scope::evaluateLocalOffsets(Scope* functionScope){
		...
		int i=0;
		while(functionScope->orderedEntries[i].isParameter==true && i<maxi){
				i++;
		}
		int offset = i+1;
		for(int j=0; j<i; j++){
		...
		functionScope->retOffset = i+2;

		//compute local variable not nested
		int k=1;
		while(functionScope->orderedEntries[i].entryType!= SymbolEntryType::SCOPE && i<maxi){
