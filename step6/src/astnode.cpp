...
string getIROpcodeString(Opcode opcode){
		string code[] =  {"ADDI", "ADDF", "SUBI", "SUBF", "MULTI", "MULTF", "DIVI", "DIVF", "STOREI", "STOREF", "GT", "GE", "LT", "LE", "NE", "EQ", "JUMP", "LABEL", "READI", "READF", "WRITEI", "WRITEF", "WRITES", "PUSH", "POP", "JSR", "LINK", "UNLINK", "RET", "HALT"};
		return code[opcode];
}
...
vector<AssemblyNode> IRNode::toAssembly(){
		...
		switch (this->opcode){
				...
				case Opcode::PUSH:
						{
								string op3reg = (findOperandType(this->op3)==OperandType::TEMP)?getReg(this->op3):this->op3;
								assembly.push_back(AssemblyNode(AssemblyIns::ASM_PUSH,op3reg,""));
								break;
						}
				case Opcode::POP:
						{
		...
		for(int i=0; i<this->idList.size(); i++){
				...
				if(isRead){
						...
						if(tmp->valType == SymbolEntryType::INT)
								...
								code.push_back(IRNode(WRITES,"","",tmp->getTempVal()));
				}
		}
...
vector<IRNode> Return::genCode(){
		...
		if(this->functionScope->retValType!= SymbolEntryType::VOID){
				...
				if(this->functionScope->retValType==SymbolEntryType::FLOAT)
						code.push_back(IRNode(Opcode::STOREF, left->getTempVal(), "",ss.str()));

				if(this->functionScope->retValType==SymbolEntryType::INT)
						code.push_back(IRNode(Opcode::STOREI, left->getTempVal(), "",ss.str()));
