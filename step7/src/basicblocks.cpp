...
vector<int> basicBlocksIndex(vector<IRNode> code){
		...
		for(int i=0; i<code.size(); i++){
                ...
                else if(ir.opcode == Opcode::JUMP||ir.opcode == Opcode::GT||ir.opcode == Opcode::GE||ir.opcode == Opcode::LT||ir.opcode == Opcode::LE||ir.opcode == Opcode::NE||ir.opcode == Opcode::EQ|| ir.opcode==Opcode::RET){
						bb.push_back(i);
				}
		}
