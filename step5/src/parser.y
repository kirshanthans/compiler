...
%type <sval> id str 
%type <ival> var_type mulop addop compop
%type <astval> stmt_list stmt base_stmt for_stmt assign_stmt assign_expr expr factor postfix_expr primary factor_prefix expr_prefix write_stmt read_stmt if_stmt cond else_part aug_stmt_list aug_stmt aug_else_part aug_if_stmt return_stmt init_stmt incr_stmt
%%
...
func_body         : decl stmt_list
				  {
						...
						cout << convertCodetoString(code, "*****Start Unoptimized IR Code*****", "*****End Unoptimized IR Code*****");
						cout << ";*****Start Unoptimized Tiny Code*****" << endl;
						unopt_assembly = genAssembly(code);
						cout << convertAssemblytoString(globalDecl,true);
						cout << convertAssemblytoString(unopt_assembly,true);
						cout << ";*****End Unoptimized Tiny Code*****" << endl;
						
						Optimizer::optimizeAll(code);
						
						cout << convertCodetoString(code, "*****Start Optimized IR Code*****", "*****End Optimized IR Code*****");
						opt_assembly = genAssembly(code);
						cout << ";*****Start Optimized Tiny Code*****" << endl;
						cout << convertAssemblytoString(globalDecl,false);
						cout << convertAssemblytoString(opt_assembly,false);
						cout << ";*****End Optimized Tiny Code*****" << endl;
				  }
				  ;

/* Statement List */
stmt_list         : stmt stmt_list{$$ = new StatementList($1,$2);}
				  | empty{$$ = new StatementList(NULL,NULL);}//or create an empty instead  
				  ;
stmt              : base_stmt{$$ = $1;}
		  		  | if_stmt{$$ = $1;}
		  		  | for_stmt{$$ = $1;}
...
write_stmt        : TOKEN_KEY_WRITE {idList.clear();}TOKEN_OP_LPAREN id_list TOKEN_OP_RPAREN TOKEN_OP_SEMICOL
				  ...
				  ;
return_stmt       : TOKEN_KEY_RETURN expr TOKEN_OP_SEMICOL{$$ = NULL;}
				  ;
